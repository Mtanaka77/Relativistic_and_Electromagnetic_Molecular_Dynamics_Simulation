...
!                       or read(12), restarting file (kstart=1)
...
!         field correction (in every 5 steps)
...
!
      ...
      implicit  none
!
      include   'param_em3p8_Ca.h'  ! <- parameter file
      include   'mpif.h'
!
...
!
      ...
      igrp = kgrp
!
      if(iflinx) then    ! 27 + 6= 33  sname='Cntemp' in param_em3p8_Ca.h
        praefixs = '/lv01/mtanaka/cntem3-para2/'//sname
        praefixi = '/lv01/mtanaka/cntem3-para2/'//cname  ! read(12) - common by NFS
        praefixc = '/lv01/mtanaka/cntem3-para2/'//cname  ! write(13)
        praefixe = '/lv01/mtanaka/cntem3-para2/'//sname  ! WRITE_CONF
      else
        praefixs = '/home/tanakam/cntem3-para2/'//sname
...
!
      integer*4  size,cl_first,MPIerror
      real*8     walltime,walltime0,buffer1,buffer2
      save       walltime0
!
...
!     +++++++++++++++++++++++++++++++++++
      if(ionode) write(11,*) 'praefix8=',praefix8
!
...
!
        ...
        xyz(i+np,1)= xyz(i,1) +0.1d-8*ranff(0.)
        xyz(i+np,2)= xyz(i,2) +0.1d-8*ranff(0.)
        xyz(i+np,3)= xyz(i,3)
        end do
!
        do i= 1,ns/2    ! for C atoms
        ns2= ns+2*np
        xyz(i+ns2,1)= xyz(i,1) +0.1d-8*ranff(0.)
        xyz(i+ns2,2)= xyz(i,2) +0.1d-8*ranff(0.)
        xyz(i+ns2,3)= xyz(i,3)
        end do
!
...
!                             ! dtwr2,dtwr3 are taken from READ_CONF, L.3710
...
!c
      if(ionode) then
!c      OPEN (unit=11,file=praefixc//'.06'//suffix2,
!c     *      status='unknown',position='append',form='formatted')
...
!
      ...
      common/parall/ nz0,nz1,nz2,nz3,i0,i1
      integer(C_INT) nw,nu,npp(10),nqq(10),i00,rank,iterft
!
...
!+++++
      logical ::  first11=.true.,first06=.true.,     &
                  ...
                  eq_phase,ifskip_e,ifskip_p,ionode
      common/sub_proc/ ionode
!
      if(ionode) then
...
!***
      rank= ipar -1
!     +++++++++++++++++  
      reg_top= num_proc  ! top register
      reg_bot= 1         ! bottom register is 1
!     +++++++++++++++++
!***
      yg0= 3.5d0*0.800d-4
...
!  bachward/forward fftw's
        plan= fftw_plan_r2r_3d  &
             (mx,my,mz, qq,qq_c,FFTW_RODFT00,FFTW_RODFT00,FFTW_RODFT00,FFTW_ESTIMATE) 
        pinv= fftw_plan_r2r_3d  &
             (mx,my,mz,psi_c,psi,FFTW_RODFT00,FFTW_RODFT00,FFTW_RODFT00,FFTW_ESTIMATE)
!
        pla1= fftw_plan_r2r_3d  &
             (mx,my,mz,cjx,cjx_c,FFTW_ReDFT00,FFTW_RODFT00,FFTW_RODFT00,FFTW_ESTIMATE) 
        pin1= fftw_plan_r2r_3d  &
             (mx,my,mz,cjx_c,cjx,FFTW_ReDFT00,FFTW_RODFT00,FFTW_RODFT00,FFTW_ESTIMATE)
        pla2= fftw_plan_r2r_3d  &
             (mx,my,mz,cjy,cjy_c,FFTW_RODFT00,FFTW_ReDFT00,FFTW_RODFT00,FFTW_ESTIMATE) 
        pin2= fftw_plan_r2r_3d  &
             (mx,my,mz,cjy_c,cjy,FFTW_RODFT00,FFTW_ReDFT00,FFTW_RODFT00,FFTW_ESTIMATE)
        pla3= fftw_plan_r2r_3d  &
             (mx,my,mz,cjz,cjz_c,FFTW_RODFT00,FFTW_RODFT00,FFTW_ReDFT00,FFTW_ESTIMATE) 
        pin3= fftw_plan_r2r_3d  &
             (mx,my,mz,cjz_c,cjz,FFTW_RODFT00,FFTW_RODFT00,FFTW_ReDFT00,FFTW_ESTIMATE)
      end if 
!
...
!  vx(n) <- vx(n+1/2) ??
      if(mod(it,5).eq.1) then ! .or. read_10) then  ! it=1, or restart
!
        do n= 1,mz
        do m= 1,my
        do l= 1,mx
...
!
!  Smoothing by FFTW
        call fftw_execute_r2r (pla1,cjx,cjx_c)
        call fftw_execute_r2r (pla2,cjy,cjy_c)
        call fftw_execute_r2r (pla3,cjz,cjz_c)
!
...
!  FFTW back in the real space
        call fftw_execute_r2r (pin1,cjx_c,cjx)
        call fftw_execute_r2r (pin2,cjy_c,cjy)
        call fftw_execute_r2r (pin3,cjz_c,cjz)
      end if
...
!
!  Small correction is required for the transverse part
!  (read my papers in Comp.Phys.Comm.2019 and 1993...)
!
...
!
        if(l.lt.1 .or. l.gt.mx .or.       & 
           ...
           n.lt.1 .or. n.gt.mz) go to 60
!
        qq(l,m,n)= qq(l,m,n) +ch(i)
   60   end do
!
!  -del^2 dphi= 4*pi*rho - div*Etx, Eq.(32)
!  match qq( ,n) and etx( ,nw); only a slice is present
!
        do nw= 1,mza
        do m=  1,my
        do l=  1,mx
...
!
!  Correction psl= charge - dE/dx
        do n= nz1(ipar),nz2(ipar)
        if(n.eq.1 .or. n.eq.mz) go to 62
!
        do m= 2,my-1
        do l= 2,mx-1
        nw= n -nz1(ipar) +1
        qx(l,m,nw)= prho*qq(l,m,n)                          &
                     - (etx(l+1,m,nw) -etx(l-1,m,nw))/hx2  &
                     - (ety(l,m+1,nw) -ety(l,m-1,nw))/hy2  &
                     - (etz(l,m,nw+1) -etz(l,m,nw-1))/hz2
        end do
        end do
   62   end do
!
!  In the backward FFTW space
!  allgather: qx( ,nw) into qq( ,n), always in mza
        call mpi_allgather (qx,mza,mpi_real8,  &
                            qq,mza,mpi_real8,  &
...
!
        fff= fnml*gamma
        psi_c(l,m,n)= fff*qq_c(l,m,n)/(akx**2 +aky**2 +akz**2)
   63   end do  
   64   end do 
   65   end do 
!
        call fftw_execute_r2r (pinv,psi_c,psi)  !! for psi
!  
!  Corrected field: etx= old etx -del*dphi, Eq.(31) of
!  Comp.Phys.Comm.2019
        do n= nz0(ipar),nz3(ipar)  !! nz0-nz3
        if(n.le.1 .or. n.ge.mz) go to 67
!
        do m= 2,my-1
        do l= 2,mx-1
        nw= n -nz1(ipar) +1
...
!
!  etx: electromagnetic field is properly added
!
      do n= nz0(ipar),nz3(ipar)  !! nz0-nz3
...
!
!  If the particles are outside the central domain
      if(l.lt.1 .or. l.ge.mx .or.  &
         m.lt.1 .or. m.ge.my .or.  &
...
!
!  If the particles are inside the domain
      else
...
!
!  Synchronize the system - only one ipar system should be !!
!  No effect if x=0 and p=0 above
!
      call mpi_allreduce (xyz(1,1),qqq(1,1),npq0,mpi_real8,mpi_sum,  &
                          mpi_comm_world,ierror)
      call mpi_allreduce (xyz(1,2),qqq(1,2),npq0,mpi_real8,mpi_sum,  &
                          mpi_comm_world,ierror)
      call mpi_allreduce (xyz(1,3),qqq(1,3),npq0,mpi_real8,mpi_sum,  &
                          mpi_comm_world,ierror)
!
      do i= 1,npq0
      xyz(i,1)= qqq(i,1)
...
!  The momentum ppp also is synchronized
      call mpi_allreduce (ppp(1,1),qqq(1,1),npq0,mpi_real8,mpi_sum,  &
                          mpi_comm_world,ierror)
      call mpi_allreduce (ppp(1,2),qqq(1,2),npq0,mpi_real8,mpi_sum,  &
                          mpi_comm_world,ierror)
      call mpi_allreduce (ppp(1,3),qqq(1,3),npq0,mpi_real8,mpi_sum,  &
                          mpi_comm_world,ierror)
!
      do i= 1,npq0
      ppp(i,1)= qqq(i,1)
...
!
        ...
        xyz(i,3)= xyz(i,3)
!
        xyz(i+np,1)= xyz(i,1) +0.1d-8*ranff(0) ! elec= H(+): ns+1 -> ns+np+1
        xyz(i+np,2)= xyz(i,2) +0.1d-8*ranff(0)
        xyz(i+np,3)= xyz(i,3)
        end do
!       ----------------
...
!
      if(iwrt3.eq.0) then
!     mxyza= mx*my*mza
        call mpi_allgather (etx(1,1,nz1(ipar)),mxyza,mpi_real8,       &
                            etx7,mxyza,mpi_real8,mpi_comm_world,ierror)
        call mpi_allgather (ety(1,1,nz1(ipar)),mxyza,mpi_real8,       &
                            ety7,mxyza,mpi_real8,mpi_comm_world,ierror)
        call mpi_allgather (etz(1,1,nz1(ipar)),mxyza,mpi_real8,       &
                            etz7,mxyza,mpi_real8,mpi_comm_world,ierror)
        call mpi_allgather (btx(1,1,nz1(ipar)),mxyza,mpi_real8,       &
                            btx7,mxyza,mpi_real8,mpi_comm_world,ierror)
        call mpi_allgather (bty(1,1,nz1(ipar)),mxyza,mpi_real8,       &
                            bty7,mxyza,mpi_real8,mpi_comm_world,ierror)
        call mpi_allgather (btz(1,1,nz1(ipar)),mxyza,mpi_real8,       &
                            btz7,mxyza,mpi_real8,mpi_comm_world,ierror)
!
!       if(ionode) then
        if(.false.) then
...
!
  ...
  880   end do
!
        unifem1(1)= sqrt(setx/(mx*my*mza))
        unifem1(2)= sqrt(setz/(mx*my*mza))
        unifem1(3)= sqrt(sbtx/(mx*my*mza))
        unifem1(4)= sqrt(sbtz/(mx*my*mza))
!
        call mpi_allreduce (unifem1,unifem2,4,mpi_real8,mpi_sum, &
                            mpi_comm_world,ierror)
!  Sqrt( **2)
        setx= unifem2(1)/size
        setz= unifem2(2)/size   
...
!  
!  Go to 1000 or to 2000
      if(mod(it,5).eq.1) then
        if(tg.gt.tmax) go to 2000 
        if(dcpu.gt.cptot) then
          if(ionode) then
            write(11,*) 'dcpu,cptot(min)=',dcpu,cptot
            write(11,*) 'it=',it
...
!
      if(istop2.ge.9999 .or. istop8.ge.999999) then 
        wrt2= 50 +ipar
        write(wrt2,*) 'Abnormal termination...'
        write(wrt2,*) '  a) istop1, istop2=',istop1,istop2
        write(wrt2,*) '  b) istop7, istop8=',istop7,istop8
...
!            npp(5)= npp(5) +1
      call mpi_allreduce (npp(1),istop10,1,mpi_integer,mpi_max, &
                          mpi_comm_world,ierror)
      call mpi_allreduce (npp(5),istop11,1,mpi_integer,mpi_max, &
                          mpi_comm_world,ierror)
...
!
      if( rank.eq.0 ) then
        call MPI_Irecv (recvbuf,lxy3,mpi_real8,num_proc-1,mpi_any_tag, &
                        mpi_comm_world,recv_request,ierror)
        call MPI_Isend (sendbuf,lxy3,mpi_real8,1,0,           &
                        mpi_comm_world,send_request,ierror)  
!
      else if( rank.eq.(num_proc-1) ) then
        call MPI_Irecv (recvbuf,lxy3,mpi_real8,num_proc-2,mpi_any_tag, &
                        mpi_comm_world,recv_request,ierror)
        call MPI_Isend (sendbuf,lxy3,mpi_real8,0,0,                &
                        mpi_comm_world,send_request,ierror)
      else
        call MPI_Irecv (recvbuf,lxy3,mpi_real8,rank-1,mpi_any_tag, &
                        mpi_comm_world,recv_request,ierror)
        call MPI_Isend (sendbuf,lxy3,mpi_real8,rank+1,0,           &
                        mpi_comm_world,send_request,ierror)
      end if
!
...
!
      if ( rank.eq.0 ) then
        call MPI_Irecv (recvbuf,lxy3,mpi_real8,1,mpi_any_tag, &
                        mpi_comm_world,recv_request,ierror)
        call MPI_Isend (sendbuf,lxy3,mpi_real8,num_proc-1,0, &
                        mpi_comm_world,send_request,ierror)  
!
      else if( rank.eq.(num_proc-1) ) then
        call MPI_Irecv (recvbuf,lxy3,mpi_real8,0,mpi_any_tag, &
                        mpi_comm_world,recv_request,ierror)
!       call MPI_Isend (sendbuf,lxy3,mpi_real8,num_proc-2,0,      &
        call MPI_Isend (sendbuf,lxy3,mpi_real8,num_proc-2,0,      &
                        mpi_comm_world,send_request,ierror)
      else
        call MPI_Irecv (recvbuf,lxy3,mpi_real8,rank+1,mpi_any_tag, &
...
!* afre table
      integer(C_INT)  naf
      parameter       (naf=(ns0/num_proc))  ! *0.3)  ! param
...
!
        do k= 1,nc3
        istop1= max0(istop1,ncel(k))  ! integer
!
...
!
!  istop2 will be used in local arrays
!          ++++++++++++++++++++++++++++++++++++ kokomade
!       istop= istop1
!       call mpi_allreduce (istop,istp,1,mpi_integer,mpi_max,  &
!                           mpi_comm_world,ierror)  ! not defined
!       istop2= istp
!
        istop2= istop1
        if(istop2.ge.nbxs) then
!
          istop2= 9999
          return
        end if
...
!       ********
!
        istop7= nafl  ! nafl exist, but < 1.7e05
        istop= nafl
        go to 770
!
!* exit if overflow for local arrays
  777   continue
        istop7= nbxc*naf  ! hard limit; > 1.7e05 
        istop= nbxc*naf
!
  770   call mpi_allreduce (istop,istp,1,mpi_integer,mpi_max, &
                            mpi_comm_world,ierror)
        istop8= istp
        if(istop8.ge.nbxc*naf) then 
!               Quit if nafl > nbxc*naf  
!
          istop8= 999999
          return
        end if
...
!
      npar3= 3*npq0
      call mpi_allreduce (ffr,ffc,npar3,mpi_real8,mpi_sum,  &
                          mpi_comm_world,ierror)
!
      do i= 1,npq0
...
!*********************************************************************
      ...
      write(7,'(34H# Boundary condition.. 0 is open: ,i16)')ifrefl
      write(7,'(34H# Maximum cpu time of run.......: ,f16.2)')cptot
      write(7,'(34H# Physikalische Endzeit.........: ,d20.6)')tmax
      write(7,'(34H# Diskretisierungs-Schritt......: ,d20.6)')dt
      write(7,'(34H# Write out interval for IWRT1..: ,d20.6)')dtwr 
      write(7,'(34H# Write out interval for IWRT2..: ,d20.6)')dtwr2
      write(7,'(34H# Write out interval for IWRT3..: ,d20.6)')dtwr3
!
      write(7,'(34H# Number of ita..bs.............: ,i16)')itabs
      write(7,'(34H# Number of np..................: ,i16)')np
!
      write(7,'(34H# carbon: charge state..........: ,f16.2)')fchar
      write(7,'(34H# Gold:   charge state..........: ,f16.2)')fcharA
!
      write(7,'(34H# Number of nZ..................: ,i16)')nZ
      write(7,'(34H# Number of nZA.................: ,i16)')nZA
      write(7,'(34H# Mass ratio: proton/electron...: ,d20.6)')massi
!
      write(7,'(34H# Laser intensity W/cm^2........: ,d20.6)')intens
      write(7,'(34H# Wavelength lambda 8.d-7 cm....: ,d20.6)')lambda
!  
      write(7,'(34H# Size of target (cm)...........: ,d20.6)')R_sp
      write(7,'(34H# Density of target (cm^-3).....: ,d20.6)')D_sp
!   --------------------
      write(7,'(34H# Ortsraum-Cutoff (Ang).........: ,d20.6)')rcut_Clf
      write(7,'(34H# LJ-Cutoff (Ang)...............: ,d20.6)')rcutlj
      write(7,'(34H# epsLJ of C (eV)...............: ,d20.6)')epsCLJ
      write(7,'(34H# epsLJ of others (eV)..........: ,d20.6)')epsLJ
      write(7,'(34H# Electron temperature..........: ,d20.6)')Temp
      write(7,'(34H# Size of rgmax.................: ,d20.6)')rgmax
!*********************************************************************
      close(07)
!
      return
...
!* C
      ...
      i= i +1
!
      xyz(i,1)= xyz(j,1) +0.1d-8*ranff(0.) !sgn(1,k)*(rd_CP +rd_el)
      xyz(i,2)= xyz(j,2) +0.1d-8*ranff(0.) !sgn(1,k)*(rd_CP +rd_el)
      xyz(i,3)= xyz(j,3) +0.1d-8*ranff(0.) !sgn(1,k)*(rd_CP +rd_el)
      end do
      end do
!
!* Au
...
!-------------------------------------------------------------
      ...
      include  'param_em3p8_Ca.h'
!
      real*8     xyz(npq0,3),vvv(npq0,3),vv
...
! page 3
      ...
      call number (6.0,3.0,0.7,ekin,0.,5)
...
!
!------------------------------------------------------------------
      subroutine ppl3da (xyz,ch,ag,Rgy1,Rgy2,ns,np,nq,  &
                         nCLp,igrp,ifskip_e,ifskip_p)
!------------------------------------------------------------------
      include    'param_em3p8_Ca.h'
!
      ...
      common/iroha/ nZ,nZA
!cc
      real*8     xyz(npq0,3),ch(npq0),ag(npq0)
...
!
!------------------------------------------------
      integer*4 function iwrta (t,twr)
!------------------------------------------------
      use, intrinsic :: iso_c_binding 
      implicit none
!
...
!
!------------------------------------------------
      integer*4 function iwrtb (t,twr)
!------------------------------------------------
      use, intrinsic :: iso_c_binding 
      implicit none
!
...
!
!------------------------------------------------
      integer*4 function iwrtc (t,twr)
!------------------------------------------------
      use, intrinsic :: iso_c_binding 
      implicit none
!
...
!
!------------------------------------------------
      integer*4 function iwrtd (t,twr)
!------------------------------------------------
      use, intrinsic :: iso_c_binding 
      implicit none
!
...
!
!------------------------------------------------
      double precision function ranff (x)
!------------------------------------------------
!*  ranf= (0,1)
      use, intrinsic :: iso_c_binding 
      implicit none
