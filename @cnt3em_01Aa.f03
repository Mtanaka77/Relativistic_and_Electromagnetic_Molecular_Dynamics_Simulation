...
!
!    l.1026  write() it,dcppu  <- comment out
!    Exec time= 1 min          <- run7s.sh: =240m
!
!  mpifrtpx -Kfast,openmp -o aq4.out @cnt3emq.f03 -lfftw3  Fujitsu
...
!     parameter  (sname='Cntemp',cname='cntemp')   ! in param_em7Aa.h
...
!
      if(iflinx) then
        ...
        praefixi = '/lv01/mtanaka/cntem3/'//cname  ! read(10) - common by NFS
        ...
        praefixi = '/data/sht/tanakam/'//cname     ! read(10) - common by NFS
...
!
!* CNT  72 cycles
!   double rings at 30, 60 Ang
!   66,528 for C
!       OPEN (unit=21,file='p_config_ss.xyz_D150',form='formatted')
!       OPEN (unit=21,file='p_config_ms.xyz_D120',form='formatted')
        OPEN (unit=21,file='p_config_ms.xyz_D100',form='formatted')
!
        read(21,'(i6)') ns1
        read(21,'(a2)') dummy
!
...
!       ++++++++++++++++
!
!       do i= ns+1,ns+np   ! electron for proton
!       xg(i+np)= xg(i) +0.1d-8*ranff(0.)
!       yg(i+np)= yg(i) +0.1d-8*ranff(0.)
!       zg(i+np)= zg(i)
!       end do
!
        do i= 1,ns/2    ! electron for C
        ns2= ns+2*np
...
!------------------------------------------
        OPEN (unit=10,file=praefixi//'.10'//suffix1,   &
              status='old',form='unformatted')
!
        read(10) it,is,ns,np,nq,nCLp,itab,item3           ! nCLp
        read(10) xg,yg,zg,px,py,pz,ch,am,ag
        read(10) etx,ety,etz,btx,bty,btz
! 
        read(10) x0,y0,z0
        read(10) fchar,fcharA,heavy,heavyA,massi,nZ,nZA   ! nZ,nZA
        read(10) R_cnt1,Z_cnt1,R_cnt2,Z_cnt2a,Z_cnt2b
!
        read(10) pi,tg,dt,dth,pthe,tmax0        ! dth; prefC_LJ,pref_LJ
        read(10) t,phi,tht,dtwr,dtwr2,dtwr3
        read(10) ekin,ppot,ekn1,ekn2,etot,Rgyi,Rgye,        &
                 Rgyf,Rgyc,Rion,Rele,dPot,ecr,elj,          &
                 sex,sez,sbx,sbz,slx,time
!
        read(10) iwa,iwb,iwc,iwd
        read(10) r_sp,d_sp,n_sp,ch_ion,wt_ion,rd_cp,rd_hp,  &
                 ch_el,wt_el,rd_el 
        read(10) W_1p,Rele0
        close(10)
!
...
!
      ...
      real*8     xx0,yy0,zz0,etx,ety,etz,btx,bty,btz,       &
                 ...
                 prho,fnml,akx,aky,akz,fff,gamma,gam,       &
                 gax,gay,gaz
      common/itre/ item3ab,item3
      common/hx2l/ Lx3,Ly3,Lz3,hx2,hy2,hz2,hx,hy,hz,p4dt,dV,cdt
      common/feld/ etx(mx,my,mz),ety(mx,my,mz),etz(mx,my,mz), &
...
!
!
!  Once at 5 steps interval. Always at it=1 or restart
!
      if(mod(it,5).eq.1 .or. read_10) then 
        if(n_twice.eq.1) then
          read_10= .false.   ! n_twice= 1, or restart by 0,1
        end if
!
        do n= 1,mz
        do m= 1,my
...
!** 10.28.2016
        if(it.eq.1 .and. ionode) then
          write(11,*) ' Wkin_E: s0(C),s1(Au),s2(el.) (per (p^2/2m))'
          write(11,*)  s0,s1,s2
          write(11,*) ' '
        end if
!
...
!
      ...
      nh2= nh1 +nZA*(ns/2)
!
      do i= ns+1,ns+nZ*(ns/2)
      ch(i)= -heavy*e_unit 
      am(i)=  heavy*m_unit !wt_el
      ag(i)= rd_el
      end do
!
      do i= ns+nZ*(ns/2)+1,nCLp
      ch(i)= -heavyA*e_unit 
...
!
        wrt2= 200 +ipar
        open(wrt2)
        write(wrt2,*) 'Sum of ch(1,2,3,4,5,6)... '
        write(wrt2,*) 'cci1=',cci1
        write(wrt2,*) 'cci2=',cci2
!       write(wrt2,*) 'cci3=',cci3
!       write(wrt2,*) 'cci4=',cci4
        write(wrt2,*) 'cci5=',cci5
        write(wrt2,*) 'cci6=',cci6
        write(wrt2,*) ' '
        write(wrt2,*) ' cci=',cci
        write(wrt2,*) ' Stop: cci is not smaller than 1.d-10...'
        write(wrt2,*) ' The error is l.2970 '
        close(wrt2)
!
        call exit
      end if
...
!
      dd= 7*ps*ag(i)
      call newcolor (3,1.,0.7,0.0)  ! gold
      call circle (xx-0.12,yy-0.12,dd,2)
  400 end do
!
!* Electrons in pellet
